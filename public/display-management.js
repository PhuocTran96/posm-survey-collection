class DisplayManagement {
    ...
    async loadDisplays(page = null, pageSize = null) {
        try {
            ...
            if (displaysResponse && displaysResponse.ok) {
                ...
                console.error('Failed to load displays:', displaysResponse ? displaysResponse.status : 'No response');
            ...
            if (statsResponse && statsResponse.ok) {
                ...
                console.error('Failed to load display stats:', statsResponse ? statsResponse.status : 'No response');
            }
    ...
    renderDisplays() {
        ...
        if (this.displays.length === 0) {
            const message = 'Chưa có bản ghi display nào trong hệ thống hoặc không có kết quả phù hợp với bộ lọc';
    ...
    handleSelectAll() {
        ...
        if (!selectAllBtn || !Array.isArray(this.displays)) return;
    ...
    async editDisplay(displayId) {
        try {
            ...
            if (response && response.ok) {
                ...
                throw new Error(errorData.message || `HTTP ${response.status}: Failed to load display record`);
            }
        } catch (error) {
    ...
    async saveDisplay() {
        ...
        try {
            ...
            const url = this.editingDisplayId ? `/api/displays/${this.editingDisplayId}` : '/api/displays';
    ...
    async confirmDelete() {
        if (!this.deleteDisplayId) return;

        try {
            this.showLoading();
            const response = await this.makeAuthenticatedRequest(`/api/displays/${this.deleteDisplayId}`, {
                method: 'DELETE'
            });
    ...
    async handleBulkDisplay() {
        if (this.selectedDisplayIds.size === 0) return;
        
        if (!confirm(`Hiển thị ${this.selectedDisplayIds.size} bản ghi đã chọn?`)) return;
    ...
    async handleBulkHide() {
        ...
        if (!confirm(`Ẩn ${this.selectedDisplayIds.size} bản ghi đã chọn?`)) return;
    ...
    async handleBulkDelete() {
        ...
        if (!confirm(`XÓA VĨNH VIỄN ${this.selectedDisplayIds.size} bản ghi đã chọn? Hành động này không thể hoàn tác!`)) return;
