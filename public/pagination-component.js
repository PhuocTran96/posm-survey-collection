...
class PaginationComponent {
    ...
    generatePageNumbers() {
        ...
        if (currentPage <= halfVisible + 1) {
            ...
            pages = [1, '...'];
            pages.push(...Array.from({ length: maxVisiblePages - 4 }, (_, i) => currentPage - Math.floor((maxVisiblePages - 4) / 2) + i));
            pages.push('...', totalPages);
        }
        ...
        if (pageInput) {
            pageInput.addEventListener('keypress', (e) => {
                if (e.key === 'Enter') {
                    ...
                    if (page >= 1 && page <= this.totalPages && page !== this.currentPage && this.onPageChange) {
                        this.onPageChange(page);
                    } else {
