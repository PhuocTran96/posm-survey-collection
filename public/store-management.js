class StoreManagement {
  ...
  bindEvents() {
    ...
    document
      .getElementById('importStoresBtn')
    ...
    // Bulk actions
    document
      .getElementById('selectAllStoresBtn')
      .addEventListener('click', () => this.toggleSelectAll());
    document
      .getElementById('bulkActivateBtn')
      .addEventListener('click', () => this.bulkUpdateStatus(true));
    document
      .getElementById('bulkDeactivateBtn')
      .addEventListener('click', () => this.bulkUpdateStatus(false));
    document
      .getElementById('bulkDeleteStoresBtn')
      .addEventListener('click', () => this.bulkDeleteStores());

    // Modal events
    document.getElementById('closeModalBtn').addEventListener('click', () => this.hideStoreModal());
    document
      .getElementById('cancelStoreBtn')
      .addEventListener('click', () => this.hideStoreModal());
    ...
    document
      ...
      .addEventListener('click', () => this.hideConfirmDialog());
  }
  ...
  async loadFilterOptions() {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to load stores for filters');
      }
  ...
  displayPagination(pagination) {
    ...
    const container =
      document.querySelector('.pagination-container') || document.createElement('div');
  ...
  handleStoreSelect(storeId, checked) {
    ...
    if (selectAllCheckbox) {
      selectAllCheckbox.indeterminate =
        this.selectedStores.size > 0 && this.selectedStores.size < this.stores.length;
  ...
  showStoreModal(store = null) {
    ...
    document.getElementById('modalTitle').textContent = this.isEditMode
      ? 'Chỉnh sửa cửa hàng'
      : 'Thêm cửa hàng mới';
  ...
  async bulkDeleteStores() {
    ...
    const confirmed = confirm(
      `Bạn có chắc chắn muốn xóa ${this.selectedStores.size} cửa hàng đã chọn?`
    );
    if (!confirmed) {
      return;
    }
  ...
  async bulkUpdateStatus(isActive) {
    ...
    const confirmed = confirm(
      `Bạn có chắc chắn muốn ${action} ${this.selectedStores.size} cửa hàng đã chọn?`
    );
    if (!confirmed) {
      return;
    }
