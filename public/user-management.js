class UserManagementApp {
  ...
  bindEvents() {
    ...
    document
      ...
      .addEventListener('click', () => this.handleBulkActivate());
    document
      .getElementById('bulkDeactivateBtn')
      .addEventListener('click', () => this.handleBulkDeactivate());
    document
      .getElementById('bulkDeleteUsersBtn')
    ...
    document
      .getElementById('role')
      .addEventListener('change', (e) => this.onRoleChange(e.target.value));
    ...
    try {
      ...
      if (usersResponse && usersResponse.ok) {
        ...
        console.error(
          ...
          usersResponse ? usersResponse.status : 'No response'
        );
  ...
  renderUsers() {
    ...
    if (this.users.length === 0) {
      const message =
        'Chưa có người dùng nào trong hệ thống hoặc không có kết quả phù hợp với bộ lọc';
  ...
  handleSelectAll() {
    ...
    if (!selectAllBtn || !Array.isArray(this.users)) {
      return;
    }
  ...
  async renderStoreTransferInterface(userAssignedStores = []) {
    ...
    this.availableStores = this.allStores.filter((store) => !assignedStoreIds.includes(store._id));
  ...
  initializeStoreTransferEvents() {
    ...
    document.getElementById('storeSearchInput').addEventListener(
      'input',
      this.debounce(() => this.applyStoreFilters(), 300)
    );
    document
      .getElementById('regionFilterSelect')
      .addEventListener('change', () => this.applyStoreFilters());
    document
    ...
    document
      ...
      .addEventListener('click', () => this.assignSelectedStores());
    document
      .getElementById('assignAllBtn')
      .addEventListener('click', () => this.assignAllVisibleStores());
    document
      .getElementById('removeSelectedBtn')
      .addEventListener('click', () => this.removeSelectedStores());
    document
      .getElementById('removeAllBtn')
      .addEventListener('click', () => this.removeAllAssignedStores());

    // Bulk action events
    document
      .getElementById('selectAllVisibleBtn')
      .addEventListener('click', () => this.selectAllVisible());
    document
      .getElementById('selectByRegionBtn')
      .addEventListener('click', () => this.selectByRegion());
    document
      .getElementById('selectAllAssignedBtn')
      .addEventListener('click', () => this.selectAllAssigned());
    document
      .getElementById('clearAllAssignedBtn')
      .addEventListener('click', () => this.clearAllAssigned());
  }
  ...
  renderAvailableStores() {
    ...
    container.innerHTML = this.filteredAvailableStores
      .map((store) => this.createStoreItemHTML(store, 'available'))
      .join('');
  ...
  renderAssignedStores() {
    ...
    if (this.assignedStoresList.length === 0) {
      container.innerHTML =
        '<div class="empty-placeholder">Chưa có cửa hàng nào được phân quyền</div>';
      return;
    }

    container.innerHTML = this.assignedStoresList
      .map((store) => this.createStoreItemHTML(store, 'assigned'))
      .join('');
  ...
  assignAllVisibleStores() {
    ...
    this.availableStores = this.availableStores.filter(
      (store) => !this.filteredAvailableStores.some((filtered) => filtered._id === store._id)
    );
    this.selectedAvailableStores.clear();
  ...
  removeSelectedStores() {
    ...
    this.assignedStoresList = this.assignedStoresList.filter(
      (store) => !selectedIds.includes(store._id)
    );
  ...
  async handleBulkActivate() {
    ...
    if (!confirm(`Kích hoạt ${this.selectedUserIds.size} tài khoản đã chọn?`)) {
      return;
    }
  ...
  async handleBulkDeactivate() {
    ...
    if (!confirm(`Khóa ${this.selectedUserIds.size} tài khoản đã chọn?`)) {
      return;
    }
  ...
  async handleBulkDelete() {
    ...
    if (
      !confirm(
        `XÓA VĨNH VIỄN ${this.selectedUserIds.size} tài khoản đã chọn? Hành động này không thể hoàn tác!`
      )
    ) {
      return;
    }
